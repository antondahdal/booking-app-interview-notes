...
## Week 5 Day 5 — Remove Duplicates II + Palindrome
...
#### LC 82 Remove Duplicates from Sorted List II (Medium) — overtime, then he rewrote
...
**Memorize this:** dummy in front of head.
`left` on dummy.
`right` on head.
...
#### LC 234 Palindrome Linked List (Easy) — on-time, he coded
...
**Memorize this:** `while (fast.next != null && fast.next.next != null)`.
Then `slow = slow.next`.
...
> #82: dummy in front.
Skip a whole equal run, then hook.
Do not sit on a duplicate.
Inner loop is still O(n).
#234: middle, flip the back half, compare.
`before` is the back head.
Not Reorder.
