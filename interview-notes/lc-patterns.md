...
**Week 4 Day 4:** **#383** HashMap counts — spend magazine; extras allowed (`<=`, not `#242 ==`). **#73** HashSet poisoned rows/cols, then wipe; in place ≠ O(1); not sliding window. Deleted **#290** that day. **Part 1 Design = Chapter 12** circuit breaker. Open = **503** try later, never **201**. One dead pod ≠ open Event. Still Part 1, **not** Part 3 (W5 Wed).  
**Week 4 Day 5:** skipped **#202**. **#290** Word Pattern — HashMap glue both ways (same as #205), not counts. `String` **`.equals`**, not `!=`. Match = keep going. `containsValue` is O(n) per step. **#76** still skipped. Friday = coding only, no LC-SD.  
**Week 5 Day 1:** **#19** two pointers with gap **n**, not a sliding window. Head victim: front already `null` after n steps → `return head.next`. Dummy = one path. **#206** behind starts **null**; save next, point back, step. Neighbor swap is **#24**. **Part 1 Design = Chapter 4** cache/TTL.  
**Week 5 Day 5:** **#82** two pointers, not HashSet (set keeps the first copy; this drops every copy). Dummy in front. Skip a whole equal run, then `left.next = right`. Inner loop is still O(n): `right` only walks forward. **#234** middle + flip back half + compare. Not Reorder (do not weave). Reverse loop is `while (slow != null)`. `before` is the back head. Friday = coding only, no LC-SD.  
...
*(Coach protocol + solutions: [LC-Practice](https://github.com/antondahdal/LC-Practice) `AI-TEACHING.md`.)*
